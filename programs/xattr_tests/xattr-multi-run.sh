...
f_runtest()
{
	...
	do
		...
		do
			echo -e "Testing Binary:\t\t${SUDO} ${MPIRUN} ${MPIROOTOPT} ${MPI_PLS_AGENT_ARG} ${MPI_MCA_BTL} ${MPI_MCA_BTL_IF} -np ${MPI_RANKS} --host ${MPI_HOSTS} ${XATTR_TEST_BIN} -i 1 -x 20 -n ${namespace} -t ${filetype} -l 50 -s 200 ${WORKPLACE}">>${LOG_FILE}
			echo "********${namespace} mode on ${filetype}********">>${LOG_FILE}

			${SUDO} ${MPIRUN} ${MPIROOTOPT} ${MPI_PLS_AGENT_ARG} ${MPI_MCA_BTL} ${MPI_MCA_BTL_IF} -np ${MPI_RANKS} --host ${MPI_HOSTS} ${XATTR_TEST_BIN} -i 1 -x 20 -n ${namespace} -t ${filetype} -l 50 -s 200 ${WORKPLACE}>>${LOG_FILE} 2>&1
			rc=$?
			if [ "$rc" != "0" ];then
